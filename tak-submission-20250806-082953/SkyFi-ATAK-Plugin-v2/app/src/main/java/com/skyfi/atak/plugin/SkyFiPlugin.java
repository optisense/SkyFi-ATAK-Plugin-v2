...
public class SkyFiPlugin implements IPlugin, MainRecyclerViewAdapter.ItemClickListener {
    ...
    // Broadcast receivers to unregister later
    private Orders ordersReceiver;
    private NewOrderFragment newOrderReceiver;
    private ArchiveSearch archiveSearchReceiver;
    private ArchivesBrowser archivesBrowserReceiver;
    private TaskingOrderFragment taskingOrderReceiver;
    private Profile profileReceiver;
    private OrderUtility orderUtilityReceiver;

    public SkyFiPlugin() {
        Log.d(LOGTAG, "SkyFiPlugin default constructor - deferring initialization to onStart()");
        // All initialization moved to onStart() for better compatibility
    }

    // Secondary constructor for backward compatibility - but not recommended for TAK.gov builds
    public SkyFiPlugin(IServiceController serviceController) {
        Log.d(LOGTAG, "SkyFiPlugin parameterized constructor - storing serviceController for onStart()");
        this.serviceController = serviceController;
        // Defer all initialization to onStart() to avoid early initialization issues
    }


    @Override
    public void onStart() {
        Log.d(LOGTAG, "onStart called - performing full plugin initialization");
        
        // Initialize services and context if not already done
        if (serviceController == null) {
            // For TAK.gov builds, we need to get the service controller through the plugin system
            Log.d(LOGTAG, "ServiceController not available, initializing through plugin context");
            // This approach works for both TAK.gov and Play Store ATAK
            initializePluginContext();
        } else {
            Log.d(LOGTAG, "ServiceController available from constructor");
            initializeWithServiceController();
        }
        
        // Initialize API client now that context is available
        try {
            apiClient = new APIClient().getApiClient();
            Log.d(LOGTAG, "API client initialized successfully");
        } catch (Exception e) {
            Log.e(LOGTAG, "Failed to initialize API client in onStart", e);
        }
        
        // Get MapView for plugin operations  
        mapView = MapView.getMapView();
        if (mapView != null) {
            Log.d(LOGTAG, "MapView available, initializing plugin components");
            ...
            // Add toolbar item after everything is initialized
            if (uiService != null && toolbarItem != null) {
                Log.d(LOGTAG, "Adding toolbar item to ATAK UI");
            ...
            Log.e(LOGTAG, "MapView not available, cannot initialize plugin");
        }
    }
    
    /**
     * Initialize plugin context when ServiceController is not available (TAK.gov builds)
     */
    private void initializePluginContext() {
        Log.d(LOGTAG, "Initializing plugin context through ATAK compatibility layer");
        
        // Get context from MapView if available
        if (MapView.getMapView() != null) {
            pluginContext = MapView.getMapView().getContext();
            pluginContext.setTheme(R.style.ATAKPluginTheme);
            Log.d(LOGTAG, "Plugin context obtained from MapView");
            
            createToolbarItem();
            registerPreferencesFragment();
        } else {
            Log.e(LOGTAG, "Cannot initialize plugin context - MapView not available");
        }
    }
    
    /**
     * Initialize plugin using ServiceController (legacy method)
     */
    private void initializeWithServiceController() {
        Log.d(LOGTAG, "Initializing plugin with ServiceController");
        ...
        if (ctxProvider != null) {
            ...
            pluginContext.setTheme(R.style.ATAKPluginTheme);
            Log.d(LOGTAG, "Plugin context obtained from ServiceController");
        } else {
            Log.e(LOGTAG, "Failed to get plugin context provider");
    ...
    private void createToolbarItem() {
        try {
            ...
            // Expected if Looper already prepared
        }

        if (pluginContext == null) {
            Log.e(LOGTAG, "Cannot create toolbar item - plugin context is null");
            return;
        }
        ...
        try {
            toolbarItem = new ToolbarItem.Builder(
                    ...
                    .build();
            Log.d(LOGTAG, "Toolbar item created successfully");
        } catch (Exception e) {
            Log.e(LOGTAG, "Failed to create toolbar item", e);
        }
    }
     ...
     */
    private void registerPreferencesFragment() {
        try {
            ...
            Log.d(LOGTAG, "Preferences fragment registered successfully");
        } catch (Exception e) {
            Log.e(LOGTAG, "Failed to register preferences fragment", e);
        }
    }
    
    private void registerDropDownReceivers() {
        // Store receivers to unregister them later
        ordersReceiver = new Orders(mapView, pluginContext);
        AtakBroadcast.DocumentedIntentFilter documentedIntentFilter = new AtakBroadcast.DocumentedIntentFilter();
        documentedIntentFilter.addAction(Orders.ACTION);
        AtakBroadcast.getInstance().registerReceiver(ordersReceiver, documentedIntentFilter);

        newOrderReceiver = new NewOrderFragment(mapView, pluginContext, "");
        ...
        AtakBroadcast.getInstance().registerReceiver(newOrderReceiver, newOrderIntentFilter);

        archiveSearchReceiver = new ArchiveSearch(mapView, pluginContext, "");
        ...
        AtakBroadcast.getInstance().registerReceiver(archiveSearchReceiver, archiveSearchFilter);

        archivesBrowserReceiver = new ArchivesBrowser(mapView, pluginContext);
        ...
        AtakBroadcast.getInstance().registerReceiver(archivesBrowserReceiver, archivesBrowserFilter);

        taskingOrderReceiver = new TaskingOrderFragment(mapView, pluginContext);
        ...
        AtakBroadcast.getInstance().registerReceiver(taskingOrderReceiver, taskingOrderFilter);

        profileReceiver = new Profile(mapView, pluginContext);
        ...
        AtakBroadcast.getInstance().registerReceiver(profileReceiver, profileFilter);

        orderUtilityReceiver = new OrderUtility(mapView, pluginContext);
        ...
        AtakBroadcast.getInstance().registerReceiver(orderUtilityReceiver, filter);
    }
    ...
    public void onStop() {
        ...
        if (ordersReceiver != null) {
            AtakBroadcast.getInstance().unregisterReceiver(ordersReceiver);
            ordersReceiver = null;
        }
        if (newOrderReceiver != null) {
            AtakBroadcast.getInstance().unregisterReceiver(newOrderReceiver);
            newOrderReceiver = null;
        }
        if (archiveSearchReceiver != null) {
            AtakBroadcast.getInstance().unregisterReceiver(archiveSearchReceiver);
            archiveSearchReceiver = null;
        }
        if (archivesBrowserReceiver != null) {
            AtakBroadcast.getInstance().unregisterReceiver(archivesBrowserReceiver);
            archivesBrowserReceiver = null;
        }
        if (taskingOrderReceiver != null) {
            AtakBroadcast.getInstance().unregisterReceiver(taskingOrderReceiver);
            taskingOrderReceiver = null;
        }
        if (profileReceiver != null) {
            AtakBroadcast.getInstance().unregisterReceiver(profileReceiver);
            profileReceiver = null;
        }
        if (orderUtilityReceiver != null) {
            AtakBroadcast.getInstance().unregisterReceiver(orderUtilityReceiver);
            orderUtilityReceiver = null;
        }
        ...
        if (aoiVisualizationManager != null) {
            ...
            aoiVisualizationManager = null;
        }
        
        // Cleanup image cache
        ImageCacheManager cacheManager = ImageCacheManager.getInstance(pluginContext);
    ...
    private void showSettingsMenu() {
        ...
        apiKeySetting.setOnClickListener(v -> {
            ...
            String currentKey = AtakPreferences.getInstance(mapView.getContext())
                .get("skyfi_api_key", "");
