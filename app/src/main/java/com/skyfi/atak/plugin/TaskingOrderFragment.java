...
public class TaskingOrderFragment extends DropDownReceiver implements DropDown.OnStateListener, View.OnClickListener {
    ...
    // Advanced tasking parameters UI elements
    private final LinearLayout passContainer;
    private final RadioGroup frequency;
    ...
    private final CheckBox polVH;
    private final CheckBox analyticsObjectDetection;
    private final CheckBox analyticsShipIdentification;
    
    // New UI elements for v2 features
    private final RadioGroup sensorType;
    private final RadioButton sensorASAP;
    private final RadioButton sensorEO;
    ...
    private final RadioButton stereo4;
    
    // Store area for provider compatibility
    private double currentAreaKm2 = 0;
    ...
    protected TaskingOrderFragment(MapView mapView, Context context) {
        ...
        // Initialize advanced parameter UI elements
        passContainer = mainView.findViewById(R.id.pass_container);
        
        frequency = mainView.findViewById(R.id.frequency);
        frequencyOnce = mainView.findViewById(R.id.frequency_once);
        frequencyOnce.setOnClickListener(this);
        frequencyDaily = mainView.findViewById(R.id.frequency_daily);
        frequencyDaily.setOnClickListener(this);
        frequencyWeekly = mainView.findViewById(R.id.frequency_weekly);
        frequencyWeekly.setOnClickListener(this);
        
        polarizationContainer = mainView.findViewById(R.id.polarization_container);
        polHH = mainView.findViewById(R.id.pol_hh);
        polVV = mainView.findViewById(R.id.pol_vv);
        polHV = mainView.findViewById(R.id.pol_hv);
        polVH = mainView.findViewById(R.id.pol_vh);
        
        analyticsObjectDetection = mainView.findViewById(R.id.analytics_object_detection);
        analyticsShipIdentification = mainView.findViewById(R.id.analytics_ship_identification);
        
        // Initialize new v2 UI elements
        sensorType = mainView.findViewById(R.id.sensor_type);
        sensorASAP = mainView.findViewById(R.id.sensor_asap);
        sensorASAP.setOnClickListener(this);
        sensorEO = mainView.findViewById(R.id.sensor_eo);
        sensorEO.setOnClickListener(this);
        sensorSAR = mainView.findViewById(R.id.sensor_sar);
        sensorSAR.setOnClickListener(this);
        sensorADSB = mainView.findViewById(R.id.sensor_adsb);
        sensorADSB.setOnClickListener(this);
        
        taskPrioritySlider = mainView.findViewById(R.id.task_priority_slider);
        taskPrioritySlider.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            @Override
            public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                // Update priority based on slider position
                taskingOrder.setPriorityLevel(progress);
            }
        ...
        stereoContainer = mainView.findViewById(R.id.stereo_container);
        stereoOptions = mainView.findViewById(R.id.stereo_options);
        stereo1 = mainView.findViewById(R.id.stereo_1);
        stereo1.setOnClickListener(this);
        stereo2 = mainView.findViewById(R.id.stereo_2);
        stereo2.setOnClickListener(this);
        stereo4 = mainView.findViewById(R.id.stereo_4);
        stereo4.setOnClickListener(this);
    ...
    public void onClick(View view) {
        ...
        // Product Types
            ...
            if (day.isChecked()) {
                ...
                // Hide polarization options for non-SAR
                polarizationContainer.setVisibility(View.GONE);
                // Hide stereo options for non-stereo
                stereoContainer.setVisibility(View.GONE);
            }
        }
            ...
            if (sar.isChecked()) {
                ...
                // Show polarization options for SAR
                polarizationContainer.setVisibility(View.VISIBLE);
                // Hide stereo options for non-stereo
                stereoContainer.setVisibility(View.GONE);
            }
        }
            ...
            if (stereo.isChecked()) {
                ...
                // Hide polarization options for non-SAR
                polarizationContainer.setVisibility(View.GONE);
                // Show stereo options for stereo
                stereoContainer.setVisibility(View.VISIBLE);
            }
        }
        ...
        // Frequency handlers
        else if (view.getId() == R.id.frequency_once) {
            if (frequencyOnce.isChecked()) {
                taskingOrder.setFrequency("once");
            }
        }
        else if (view.getId() == R.id.frequency_daily) {
            if (frequencyDaily.isChecked()) {
                taskingOrder.setFrequency("daily");
            }
        }
        else if (view.getId() == R.id.frequency_weekly) {
            if (frequencyWeekly.isChecked()) {
                taskingOrder.setFrequency("weekly");
            }
        }
        
        // Sensor type handlers
        else if (view.getId() == R.id.sensor_asap) {
            if (sensorASAP.isChecked()) {
                taskingOrder.setSensorType("ASAP");
            }
        }
        else if (view.getId() == R.id.sensor_eo) {
            if (sensorEO.isChecked()) {
                taskingOrder.setSensorType("EO");
            }
        }
        else if (view.getId() == R.id.sensor_sar) {
            if (sensorSAR.isChecked()) {
                taskingOrder.setSensorType("SAR");
                // Auto-select SAR product type
                sar.setChecked(true);
                onClick(sar);
            }
        }
        else if (view.getId() == R.id.sensor_adsb) {
            if (sensorADSB.isChecked()) {
                taskingOrder.setSensorType("ADS-B");
            }
        }
        
        // Stereo options handlers
        else if (view.getId() == R.id.stereo_1) {
            if (stereo1.isChecked()) {
                taskingOrder.setStereoImages(1);
            }
        }
        else if (view.getId() == R.id.stereo_2) {
            if (stereo2.isChecked()) {
                taskingOrder.setStereoImages(2);
            }
        }
        else if (view.getId() == R.id.stereo_4) {
            if (stereo4.isChecked()) {
                taskingOrder.setStereoImages(4);
            }
        }
    ...
    private void collectAdvancedParameters() {
        // Collect selected polarizations for SAR
        if (sar.isChecked()) {
            java.util.List<String> polarizations = new java.util.ArrayList<>();
            if (polHH.isChecked()) polarizations.add("HH");
            if (polVV.isChecked()) polarizations.add("VV");
            if (polHV.isChecked()) polarizations.add("HV");
            if (polVH.isChecked()) polarizations.add("VH");
            taskingOrder.setPolarizations(polarizations.toArray(new String[0]));
        } else {
            taskingOrder.setPolarizations(null);
        }
        
        // Collect analytics options
        taskingOrder.setAnalyticsObjectDetection(analyticsObjectDetection.isChecked());
        taskingOrder.setAnalyticsShipIdentification(analyticsShipIdentification.isChecked());
        
        // Collect selected passes
        collectSelectedPasses();
    }
    
    private void placeOrder() {
        // Collect all advanced parameters before placing order
        ...
        // Log the complete tasking order with all parameters
        Log.d(LOGTAG, "Placing order with parameters: " + taskingOrder.toString());
    ...
    private void resetForm() {
        ...
        frequency.check(R.id.frequency_once);
        polHH.setChecked(false);
        polVV.setChecked(false);
        polHV.setChecked(false);
        polVH.setChecked(false);
        analyticsObjectDetection.setChecked(false);
        analyticsShipIdentification.setChecked(false);
        polarizationContainer.setVisibility(View.GONE);
        passContainer.removeAllViews();
        
        // Reset new v2 elements
        sensorType.check(R.id.sensor_asap);
        taskPrioritySlider.setProgress(50);
        stereoContainer.setVisibility(View.GONE);
        stereoOptions.check(R.id.stereo_1);
        taskingOrder.setSensorType("ASAP");
        taskingOrder.setPriorityLevel(50);
        taskingOrder.setStereoImages(1);
    }
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (intent.getAction().equals(ACTION)) {
            ...
            // Also update provider compatibility when area changes
            if (area > 0) {
                currentAreaKm2 = area;
                updateProviderCompatibility();
            }
            
            // Populate available passes (mock data for now)
            populateAvailablePasses();
    ...
    private void updateProviderCompatibility() {
        // This method would check area size and other constraints
        // to enable/disable providers based on their capabilities
        // For now, just log the area
        Log.d(LOGTAG, "Area size: " + currentAreaKm2 + " km²");
    }
    
    private void populateAvailablePasses() {
        // Clear existing passes
        passContainer.removeAllViews();
        
        // In a real implementation, this would query available satellite passes
        // For now, add some mock passes
        String[] mockPasses = {
            "2024-01-15 14:30 UTC - Satellite A",
            "2024-01-15 16:45 UTC - Satellite B",
            "2024-01-16 09:20 UTC - Satellite A",
            "2024-01-16 11:15 UTC - Satellite C"
        };
        
        for (String pass : mockPasses) {
            CheckBox passCheckBox = new CheckBox(context);
            passCheckBox.setText(pass);
            passCheckBox.setLayoutParams(new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT,
                LinearLayout.LayoutParams.WRAP_CONTENT));
            passContainer.addView(passCheckBox);
        }
    }
    
    private void collectSelectedPasses() {
        java.util.List<String> selectedPasses = new java.util.ArrayList<>();
        for (int i = 0; i < passContainer.getChildCount(); i++) {
            View child = passContainer.getChildAt(i);
            if (child instanceof CheckBox) {
                CheckBox cb = (CheckBox) child;
                if (cb.isChecked()) {
                    selectedPasses.add(cb.getText().toString());
                }
            }
        }
        taskingOrder.setSelectedPasses(selectedPasses.toArray(new String[0]));
    }
}
