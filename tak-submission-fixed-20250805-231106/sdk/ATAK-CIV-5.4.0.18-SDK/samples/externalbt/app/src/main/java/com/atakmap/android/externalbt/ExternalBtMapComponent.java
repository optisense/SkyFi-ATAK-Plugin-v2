...
public class ExternalBtMapComponent extends DropDownMapComponent {
    ...
    private void fireGPSUpdate(final PacketRMC rmc, final PacketGGA gga) {
        ...
        if (item != null) {
            ...
            data.setMetaDouble("mockLocationSpeed",
                    (rmc.getGroundSpeed() / 1.85200)); // speed in meters per second
            data.setMetaDouble("mockLocationAccuracy", dilution); // accuracy in meters
            data.setMetaDouble("mockLocationBearing", trackAngle);

            data.setMetaString("locationSourcePrefix", "mock");
            data.setMetaBoolean("mockLocationAvailable", true);

            data.setMetaString("mockLocationSource", "Plugin Supplied GPS");
            ...
            data.setMetaInteger("mockFixQuality", fixQuality);
