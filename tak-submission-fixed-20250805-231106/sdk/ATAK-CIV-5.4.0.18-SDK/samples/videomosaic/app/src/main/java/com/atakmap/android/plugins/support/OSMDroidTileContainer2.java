...
public final class OSMDroidTileContainer2 implements TileContainer2 {
    ...
    public final static TileContainerSpi SPI = new TileContainerSpi() {
        ...
        public TileContainer create(String name, String path, TileMatrix spec) {
            ...
            final File f = (path != null) ? new File(path) : null;
            ...
            try {
                ...
                final TileContainer retval =  new OSMDroidTileContainer2(name, spec.getSRID(), db);
                db = null;
                return retval;
            } finally {
                if(db != null)
        ...
        public TileContainer open(String path, TileMatrix spec, boolean readOnly) {
            ...
            try {
                ...
                final TileContainer retval =  new OSMDroidTileContainer2(info.provider, info.srid, db);
                db = null;
                return retval;
            } finally {
                if(db != null)
        ...
        public boolean isCompatible(TileMatrix spec) {
            return OSMDroidTileContainer2.isCompatible(spec);
        }
    };
    ...
    private OSMDroidTileContainer2(String name, int srid, DatabaseIface db) {
        this.name = name;
        switch(srid) {
            case 4326 :
    ...
    public void setTile(int level, int x, int y, Bitmap data, long expiration) throws TileEncodeException {
        ...
        setTile(level, x, y, blob, expiration);
    }
    ...
    public synchronized void deleteTiles(int zoomLevel, int stx, int sty, int ftx, int fty) {
        db.beginTransaction();
        try {
            PrecompiledStatements stmts = getPrecompiledStmts();
            StatementIface stmt = null;
            try {
                if (stmts.deleteTiles == null)
                    stmts.deleteTiles = this.db.compileStatement("DELETE FROM tiles WHERE key >= ? AND key <= ?");
                stmt = stmts.deleteTiles;

                // column-wise delete
                for (int x = stx; x <= ftx; x++) {
                    stmt.clearBindings();
                    stmt.bind(1, OSMUtils.getOSMDroidSQLiteIndex(zoomLevel, x, sty));
                    stmt.bind(2, OSMUtils.getOSMDroidSQLiteIndex(zoomLevel, x, fty));
                    stmt.execute();
                }
            } finally {
                if (stmt != null) {
    ...
    public static TileContainer2 openOrCreate(String path, String provider, int srid) {
        File f = (path != null) ? new File(path) : null;
        if(f != null && IOProviderFactory.exists(f)) {
            TileContainer2 retval = (TileContainer2)SPI.open(path, null, false);
        ...
        try {
            ...
            final TileContainer2 retval =  new OSMDroidTileContainer2(provider, srid, db);
            db = null;
            return retval;
        } finally {
            if(db != null)
