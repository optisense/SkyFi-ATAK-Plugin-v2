...
public class GLExtrudedPolygonsBatchExample extends GLAbstractLayer2 {
    ...
    public final static GLLayerSpi2 SPI = new GLLayerSpi2() {
        ...
        public GLLayer2 create(Pair<MapRenderer, Layer> object) {
            ...
            if (!(layer instanceof ExtrudedPolygonsBatchExample))
                return null;

            return new GLExtrudedPolygonsBatchExample(renderCtx,
                    (ExtrudedPolygonsBatchExample) layer);
        }

    };

    private final ExtrudedPolygonsBatchExample subject;
    ...
    public GLExtrudedPolygonsBatchExample(MapRenderer surface,
            ExtrudedPolygonsBatchExample subject) {
    ...
    @Override
    protected void init() {
        // the init function allocates GL specific resources that will be used
        // during the draw pump
        super.init();

        // create our batch renderer
    ...
    protected void drawImpl(GLMapView view, int renderPass) {
        ...
        for (Polygon p : shapes) {
            ...
            this.batch.batch(-1, // texture ID, not specified
                    GLES20FixedPipeline.GL_TRIANGLE_STRIP, // mode
            ...
            for (int i = 0; i < exterior.getNumPoints(); i++) {
                ...
                // vertex at the top of the extrusion
                view.scratch.geo.set(exterior.getY(i),
                        exterior.getX(i), z + baseElevation);
                // convert the LLA coordinate to GL x,y,z
                view.currentPass.scene.forward(view.scratch.geo, view.scratch.pointD);
                // set the 3 float values at the pointer
                Unsafe.setFloats(pVerts,
            ...
            switch (GLTriangulate.triangulate(this.vertices,
                ...
                case GLTriangulate.INDEXED:
                    this.indices.flip();
                    this.batch.batch(
                            -1, // texture ID, not specified
                            GLES20FixedPipeline.GL_TRIANGLES, // mode
                            3, // number of elements per vertex coordinate
                            0, this.vertices, // vertex coordinates
                            0, null, // texture coordinates, not specified
        ...
        // all content has been added to the batch, end to flush to the display
        this.batch.end();
    }

    @Override
    public void release() {
        // the release method releases any resources allocated for rendering
        // purposes as a result of previous invocation of the draw method
        super.release();
        ...
        if (this.batch == null) {
            ...
            // the dispose method frees the associated native resources. if we
            // do not explicitly call, this will be done at finalization
            this.batch.dispose();
            this.batch = null;
        }
