...
public class ElevationCache {
    private final DatabaseIface database;
    private final int tileWidth;
    private final int tileHeight;

    public ElevationCache(String file) {
        database = Databases.openOrCreateDatabase(file);
        database.execute(
                "CREATE TABLE IF NOT EXISTS dataset (path TEXT, tilewidth TEXT, tileheight TEXT)",
                null);
        database.execute(
                "CREATE TABLE IF NOT EXISTS tiles (level INTEGER, tilecol INTEGER, tilerow INTEGER, minval REAL, maxval REAL, nodata INTEGER, width INTEGER, height INTEGER, data BLOB)",
                null);
        this.tileWidth = 512;
        this.tileHeight = 512;
    ...
    public double getValue(Dataset dataset, int level, double x, double y) {
        ...
        final int tileColumn = ((int) MathUtils.clamp(Math.round(x), 0,
                dataset.GetRasterXSize() - 1) / this.tileWidth) >> level;
        final int tileRow = ((int) MathUtils.clamp(Math.round(y), 0,
                dataset.GetRasterYSize() - 1) / this.tileHeight) >> level;
    ...
    private double getValue(Tile tile, int x, int y) {
        final int tx = ((x - (tile.tileColumn << tile.level) * this.tileWidth)
                / (1 << tile.level));
        final int ty = ((y - (tile.tileRow << tile.level) * this.tileHeight)
                / (1 << tile.level));
        ...
        if (val == 0xFF)
            return Double.NaN;
        return (((double) val / 254d) * (tile.maxValue - tile.minValue))
                + tile.minValue;
    }
    ...
    private Tile getTile(Dataset dataset, int level, int tileColumn,
            int tileRow) {
        // query for the tile
        QueryIface result = null;
        try {
            result = this.database.compileQuery(
                    "SELECT minval, maxval, nodata, width, height, tilerow, tilecol, level, data FROM tiles WHERE level = ? AND tilecol = ? AND tilerow = ? LIMIT 1");
            result.bind(1, level);
        ...
        if (false) {
            Tile ul = this.getTile(dataset, level - 1, tileColumn * 2,
                    tileRow * 2);
            Tile ur = this.getTile(dataset, level - 1, tileColumn * 2 + 1,
                    tileRow * 2);
            Tile ll = this.getTile(dataset, level - 1, tileColumn * 2,
                    tileRow * 2 + 1);
            Tile lr = this.getTile(dataset, level - 1, tileColumn * 2 + 1,
                    tileRow * 2 + 1);
            ...
            if (ul.noData && ur.noData && ll.noData && lr.noData) {
                ...
                retval = new Tile();
                retval.minValue = MathUtils.min(ul.minValue, ur.minValue,
                        ll.minValue, lr.minValue);
                retval.maxValue = MathUtils.max(ul.maxValue, ur.maxValue,
                        ll.maxValue, lr.maxValue);
                retval.noData = false;
                retval.level = level;
                retval.tileColumn = tileColumn;
                retval.tileRow = tileRow;
                        ...
                        for (int y = 0; y < this.tileHeight / 2; y++) {
                            for (int x = 0; x < this.tileWidth / 2; x++) {
                                final int fully = (y
                                        + ((this.tileHeight / 2) * (i / 2)));
                                final int fullx = (x
                                        + ((this.tileWidth / 2) * (i % 2)));
                                final int idx = (fully * retval.width) + fullx;
                                // XXX - expects x/y as full image coords
                                final double value = getValueInterpolate(
                                        tiles[i], (x * 2) + 0.5, (y * 2) + 0.5);
                                if (Double.isNaN(value)) {
                                    retval.data[idx] = (byte) 0xFF;
                                } else {
                                    retval.data[idx] = (byte) MathUtils
                                            .clamp((((value - retval.minValue)
                                                    / (retval.maxValue
                                                            - retval.minValue))
                                                    * 254d), 0d, 254d);
                                }
            ...
            final double noDataValue = (ndv[0] != null) ? ndv[0].doubleValue()
                    : Double.NaN;

            GdalTileReader reader = new GdalTileReader(dataset,
                    dataset.GetDescription(), this.tileWidth, this.tileHeight,
                    null, TileReader.getMasterIOThread());
                ...
                for (int y = 0; y < dstH; y++) {
                    ...
                    for (int x = 0; x < dstW; x++) {
                        ...
                        if (Double.isNaN(sample))
                            ...
                            retval.data[dstIdx++] = (byte) (((sample
        ...
        try {
            stmt = this.database.compileStatement(
                    "INSERT INTO tiles (level, tilecol, tilerow, minval, maxval, nodata, width, height, data) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    private static abstract class Sampler {
        ...
        public void readSamples(double[] buffer, Dataset dataset,
                double noDataValue, int srcX, int srcY, int srcW, int srcH,
                int dstW, int dstH, SampleStatistics stats) {
            final int numSamples = (dstW * dstH);
            ByteBuffer raw = ByteBuffer
                    .allocateDirect(numSamples * getSampleSize());
            raw.order(ByteOrder.nativeOrder());

            dataset.ReadRaster_Direct(srcX, srcY, srcW, srcH, dstW, dstH,
                    this.getSampleType(), raw, new int[] {
                            1
                    }, 0);
            raw.clear();
    ...
    private final static Sampler Uint8Sampler = new Sampler(gdalconst.GDT_Byte,
            1) {
        @Override
    ...
    private final static Sampler Uint16Sampler = new Sampler(
    ...
    private final static Sampler Int16Sampler = new Sampler(gdalconst.GDT_Int16,
            2) {
        @Override
        public double nextSample(ByteBuffer buffer) {
    ...
    private final static Sampler Uint32Sampler = new Sampler(
    ...
    private final static Sampler Int32Sampler = new Sampler(gdalconst.GDT_Int32,
            4) {
        @Override
        public double nextSample(ByteBuffer buffer) {
    ...
    private final static Sampler Float32Sampler = new Sampler(
            gdalconst.GDT_Float32, 4) {
        @Override
        public double nextSample(ByteBuffer buffer) {
            return buffer.getFloat();
        }
    };
    private final static Sampler Float64Sampler = new Sampler(
            gdalconst.GDT_Float64, 8) {
        @Override
        public double nextSample(ByteBuffer buffer) {
