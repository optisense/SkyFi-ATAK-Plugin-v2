...
import com.atakmap.android.hellobuildings.opengl.examples.GLBuildingCacheBatch;
import com.atakmap.android.hellobuildings.opengl.examples.GLBuildingRenderBatch;
import com.atakmap.android.hellobuildings.opengl.examples.GLExtrudingPolygon;
...
public class GLOSMBuildingsLayer
    ...
    public final static GLLayerSpi2 SPI = new GLLayerSpi2() {
        ...
        public GLLayer2 create(Pair<MapRenderer, Layer> object) {
            ...
            return new GLOSMBuildingsLayer(renderCtx, (BuildingsExample) layer);
        }
    };
    ...
    protected boolean updateRenderList(
        ...
        // query
        final Map<Long, ExtrudedPolygonData> toRelease = new HashMap<Long, ExtrudedPolygonData>(
                this.polys);
        polys.clear();
        for (Feature f : pendingData) {
            ExtrudedPolygonData renderer = toRelease.remove(f.getId());
            if (renderer == null) {
                ...
                renderer = new ExtrudedPolygonData();
            }

            renderer.updateGeometry(f);
            polys.put(f.getId(), renderer);
        }
