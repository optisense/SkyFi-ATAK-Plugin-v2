...
import com.atakmap.android.commout.plugin.R;
...
public class CommoutMapComponent extends AbstractMapComponent {
    ...
    private void send(final byte[] data) {
        ...
        try {
            socket = SocketFactory.getSocketFactory().createMulticastSocket(
                    outputPort); // status
        ...
        if (socket != null) {
            ...
            try {
                ...
                socket.send(packet);
                Log.d(TAG, "sending " + data + " to " + outputAddress + ":"
                        + outputPort);
            } catch (IOException e) {
                Log.e(TAG,"error",e);
            }
    ...
    private void toMapItem(byte[] data) {
        ...
        if (mi == null) {
            ...
            PlacePointTool.MarkerCreator mc = new PlacePointTool.MarkerCreator(
                    GeoPoint.parseGeoPoint(sarray[2]));
            mc.setUid(sarray[1]);
    ...
    private byte[] fromMapItem(PointMapItem mi) {
        ...
        String retVal = type + "|" + mi.getUID() + "|" + mi.getPoint() + "|"
                + mi.getMetaString("callsign", "nonset");
